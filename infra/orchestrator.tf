# 1. Inicialização do Workspace (Puxa o código do Git)
resource "null_resource" "workspace_init" {
  provisioner "local-exec" {
    command = <<EOT
      ...
      curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d "{}" "https://dataform.googleapis.com/v1beta1/$REPO_PATH/workspaces?workspaceId=main-workspace" || echo "Workspace OK"
